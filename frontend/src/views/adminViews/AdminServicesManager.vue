...
const fetchAndFill = async () => {
  ...
  try {
    services.value = await fetchAllComuni();
  } catch (e) {
...
const submitForm = async () => {
  ...
  try {
    if (!editingId.value) {
      await createComune({ ...formData });
    } else {
      await updateComune(editingId.value, { ...formData });
    }
    await fetchAndFill();
    resetForm();
  } catch (e) {
